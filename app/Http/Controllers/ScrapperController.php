...
class ScrapperController extends Controller
{
    ...
    public function scrapFacebook($keyword)
    {
        ...
        $token = config('laravel-facebook-sdk.facebook_config.app_id').'|'.config('laravel-facebook-sdk.facebook_config.app_secret');

        try {
            $responses = $fb->get('/search?q='.$keyword.'&type=place&access_token='.$token)->getDecodedBody();
            return $responses;
